...
full test suite runs without a real Postgres instance, no NATS_URL so the
event publisher always uses the logging fallback, and a clean BarProvider
seam per test."""
...
def _reset_seams():
    ...
    providers.set_bar_provider(None)
    yield
    events.set_publisher(None)
    providers.set_bar_provider(None)
