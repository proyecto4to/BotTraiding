"""AI-engine event publishing seam (docs/ARCHITECTURE.md seccion 7).

Recommendations and regime updates are published to NATS when NATS_URL
is configured AND reachable; otherwise a logging no-op publisher is used
so the API never depends on the event bus (tests run on the fallback).
Same pattern as strategy-engine's signal.created seam.
"""
...
logger = logging.getLogger("ai-engine.events")

RECOMMENDATION_CREATED_SUBJECT = "ai.recommendation.created"
REGIME_UPDATED_SUBJECT = "ai.regime.updated"
...
def build_publisher() -> EventPublisher:
    ...
    if not url:
        logger.info("NATS_URL not set; ai events use logging fallback")
        return LoggingEventPublisher()
    try:
        import nats  # noqa: F401
    except ImportError:
        logger.warning("nats-py not installed; ai events use logging fallback")
        return LoggingEventPublisher()
